...
describe("buildHeatmap",()=>{
  ...
  it("builds complete calendar weeks for the month view and identifies today",()=>{const model=buildHeatmap(activity,"month",new Date("2026-08-15T12:00:00"));expect(model.cells.length%7).toBe(0);expect(model.today).toBe("2026-08-15");expect(model.cells.find(item=>item?.date==="2026-08-03")).toMatchObject({created:1,updated:2,count:3})});
