...
import { quickCreateElement, type QuickCreateDirection } from "../editor/quickCreate";
...
function ExcalidrawEditor({initial,user}:{initial:Drawing;user:User}){
  ...
  useEffect(()=>{const target=root.current;if(!apiRef||!target)return;const onKeyDown=(event:KeyboardEvent)=>{if((!event.ctrlKey&&!event.metaKey)||event.altKey||!["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"].includes(event.key))return;const node=event.target as HTMLElement|null;if(node?.matches("input, textarea, [contenteditable=true]"))return;event.preventDefault();event.stopPropagation();quickCreateElement(apiRef,event.key as QuickCreateDirection)};target.addEventListener("keydown",onKeyDown,true);return()=>target.removeEventListener("keydown",onKeyDown,true)},[apiRef]);
