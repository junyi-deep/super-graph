      ...
      .then((item) =>
        setDrawing(
          item.type === "excalidraw"
            ? { ...item, scene: restoreSceneFromStorage(item.scene) }
            : item,
        ),
      )
...
function ExcalidrawEditor({ initial, user }: { initial: Drawing; user: User }) {
  ...
  const saveName = async () => {
    ...
    if (!name || name === drawing.name) return;
    try {
      setDrawing(await api.rename(id, name));
    } catch (error) {
      apiRef?.setToast({
  ...
  const createPNG = () =>
    exportToBlob({ ...data(), mimeType: "image/png", maxWidthOrHeight: 8192 });
  const createSVG = async () => {
    const svg = await exportToSvg({ ...data(), renderEmbeddables: true });
    ...
    <div className="editor-page">
      <header className="editor-bar">
        ...
        <div className="filename-wrap">
          <FilePenLine className="file-glyph" size={19} />
          {editingName ? (
            <input
              autoFocus
              value={nameDraft}
        ...
        <details className="collaborator-menu">
          <summary>
            <span className={collab.connected ? "online-dot" : "offline-dot"} />
            {collab.connected
              ? `协作已连接 · ${collab.users.length}`
              : "协作重连中"}
            <b>⌄</b>
          </summary>
          <div>
            {collab.users.length ? (
              collab.users.map((person) => (
                <p key={person.id}>
      ...
      <div className="editor-body">
        <EditorFileSidebar user={user} selectedId={id} />
        <div className="canvas" ref={root}>
          <Excalidraw
            initialData={scene as any}
            excalidrawAPI={setAPI}
            onChange={onChange}
            onPointerUpdate={collab.onPointerUpdate}
            isCollaborating={collab.connected}
